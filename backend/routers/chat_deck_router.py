...
from models import ChatsDeckDB

router = APIRouter()

#--- list all themes)
@router.get("/", response_description="List all chat decks ")
async def list_available_chat_decks(request:Request, language_uid: Optional[str]=None)-> List[ChatsDeckDB]:
    query={}
    if language_uid:
        query["language"] = language_uid
    full_query = request.app.mongodb["langdeckAB"].find(query).sort("language",1)
    result = [ChatsDeckDB(**raw_rec) async for raw_rec in full_query]
    return result
