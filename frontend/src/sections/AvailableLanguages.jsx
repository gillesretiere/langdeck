...
const AvailableLanguages = ({ languages }) => {
    return (
        <>
            <div className='text-xl sm:text-2xl font-artifex_cf font-bold tracking-tight text-gray-900 mt-5' >Déjà disponibles</div>

            <div className={`mt-1 grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 sm:gap-4 gap-3`}>
                {languages.filter(item => item.available).map((language, index) => (
