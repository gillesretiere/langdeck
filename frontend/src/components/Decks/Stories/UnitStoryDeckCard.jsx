...
import UnitStoryDeckCardStack from './UnitStoryDeckCardStack';


const UnitStoryDeckCard = ({ unit_story, img }) => {
  let ctx = useContext(DeckContext);
  const { story_order, story_name, story_translation, story_illustration, story_desc, story_desc_translation, story_synopsis, story_synopsis_translation, story_language, story_translation_id, } = unit_story;
  // console.log (params);
  // console.log (scene);
  ...
  return (
    <div className={`h-100 mx-1 ${classes.card__container}`}>
      <div className={`${classes.card__wrapper}`}>
        <Link to={{ pathname: `/saynete_player_page/${story_translation_id}` }} onClick={linkHandler} >
          ...
          <div className={`flex flex-col`}>
            <div className={`inline-grid grid-cols-[30%_4fr] gap-0.5 text-left`}>
              <div className={`${classes.card__description_title}`}>{story_order}</div>
              <div>
            ...
            <div className={`inline-grid grid-cols-[30%_2fr_2fr] gap-0.5 ${classes.card__footer}`}>
      ...
      {/* <UnitStoryDeckCardStack unit_story={unit_story}></UnitStoryDeckCardStack> */}
